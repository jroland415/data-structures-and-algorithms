...
const toTitleCase = (arr) => {
  return arr.map(string => string.charAt(0).toUpperCase()+string.substring(1,string.length+1));
};
...
let biggerThanLuke = (arr) => {
  let characterNames = arr.reduce((acc, curr) => {
    parseInt(curr.mass) > parseInt(arr[0].mass) ? acc.push(curr.name) : acc;
    return acc;
  }, []);
