...
const count = (target, input) => {
  let concatArray = input.reduce((acc, curr) => {
    return acc.concat(curr);
  }, []);
  return concatArray.filter(num => num === target).length;
};
...
const totalSum = (input) => {
  let singleArray = input.reduce((acc, curr) => {
    return acc.concat(curr);
  }, []);
  return singleArray.reduce((acc, curr) => acc + curr);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(array => {
    let arraysByFive = array.reduce((acc, curr) => {
      (curr % 5 === 0 && typeof curr === 'number') ? acc.push(curr) : acc;
      return acc;
    }, []);
