...
const forLoopTwoToThe = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    nthArray.push(Math.pow(2,arr[i]));
  }
  return nthArray;
};
...
const forEachTwoToThe = (arr) => {
  let nthArrayForEach =[];
  arr.forEach( value => {
    nthArrayForEach.push(Math.pow(2,value));
  });
  return nthArrayForEach;
};
...
const evenOdd = (arr) => {
  let evenOddArray = arr.map( element => {
    if (element % 2 === 0) {
      return 'even';
    }
    else if (element % 2 === 1) {
...
const extractAbilities = (arr) => {
  let abilityNameArray = arr.map( snorlax => {
    return snorlax.ability.name;
  });
...
const extractStats = (arr) => {
  let snorlaxStatsArray = arr.map( snorlax => {
    return {name: snorlax.stat.name, total: snorlax.effort + snorlax.baseStat};
  });
  return snorlaxStatsArray;
};
